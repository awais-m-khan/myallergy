    ...
    <div className="bg-white rounded-2xl border border-gray-100 p-5">
      <div className="flex items-center justify-between mb-3">
        ...
        <Link to="/search" className="text-xs text-green-600 hover:text-green-700 font-medium flex items-center gap-0.5">
          {likedFoods.length > 0 ? 'Find more' : 'Search food'} <ChevronRight size={13} />
        </Link>
      </div>

      {likedFoods.length === 0 ? (
        <p className="text-sm text-gray-400 py-2">
          Search for food and tap ❤️ to save your favourites here.
        </p>
      ) : (
            ...
            {SEVERITY_ORDER.filter((s) => grouped[s]).map((s) => (
              grouped[s].map((a) => (
                <div key={a.id} className="flex items-center gap-2 py-1.5 border-b border-gray-50 last:border-0">
                  <span className={`px-2 py-0.5 rounded-full text-xs font-semibold shrink-0 ${SEVERITY_STYLES[s].badge}`}>
                    {SEVERITY_STYLES[s].label}
                  </span>
                  <span className="text-sm text-gray-900">{a.name}</span>
                  {a.exceptions && (
                    <span className="ml-auto text-xs bg-amber-50 text-amber-600 border border-amber-200 px-2 py-0.5 rounded-full shrink-0">
                      except {a.exceptions}
                    </span>
                  )}
                </div>
